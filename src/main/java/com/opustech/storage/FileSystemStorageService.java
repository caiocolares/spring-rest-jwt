...
public class FileSystemStorageService implements StorageService {
	...
	public void store(MultipartFile file,String path) {
		try {
			...
			Path completePath = Paths.get(root+"/"+path);
			Files.createDirectories(completePath);
			if(!Files.exists(completePath.resolve(file.getOriginalFilename()))){
				Files.copy(file.getInputStream(),completePath.resolve(file.getOriginalFilename()));
			}
		} catch (IOException e) {
			throw new StorageException(
					"Failed to store file " + file.getOriginalFilename(), e);
